...
#define PRINT_STATUS(type) case HAL_##type: UART_Transmit((uint8_t*)#type "\r\n"); break;

static void oled_WriteCommand(uint8_t command) {
	HAL_I2C_Mem_Write(&hi2c1, OLED_I2C_ADDR, 0x00, 1, &command, 1, 100);
}
...
void oled_UpdateScreen(void) {
	...
	for (i = 0; i < 8; i++) {
		...
		HAL_I2C_Mem_Write(&hi2c1, OLED_I2C_ADDR, 0x40, 1,
				&OLED_Buffer[OLED_WIDTH * i], OLED_WIDTH, 25);
	}
}
...
void oled_DrawSquare(uint8_t x1, uint8_t x2, uint8_t y1, uint8_t y2,
		OLED_COLOR color) {
...
char oled_WriteChar(char ch, FontDef Font, OLED_COLOR color) {
	...
	for (i = 0; i < Font.FontHeight; i++) {
		...
		for (j = 0; j < Font.FontWidth; j++) {
			if ((b << j) & 0x8000) {
				oled_DrawPixel(OLED.CurrentX + j, (OLED.CurrentY + i),
						(OLED_COLOR) color);
			} else {
				oled_DrawPixel(OLED.CurrentX + j, (OLED.CurrentY + i),
						(OLED_COLOR) !color);
...
void oled_DrawBitmap(uint8_t *bmp, uint8_t x, uint8_t y, uint8_t width,
		uint8_t height) {
